...
void PioSPI::initDma(DmaChannel txChan, DmaChannel rxChan) noexcept
{
    io_rw_8 *txfifo = (io_rw_8 *) &dev.pio->txf[dev.sm];
    io_rw_8 *rxfifo = (io_rw_8 *) &dev.pio->rxf[dev.sm];
    ...
    dma_channel_configure(txChan, &c, txfifo, nullptr, 0, false);
    c = dma_channel_get_default_config(rxChan);
    channel_config_set_transfer_data_size(&c, DMA_SIZE_8);
    channel_config_set_dreq(&c, pio_get_dreq(dev.pio, dev.sm, false));
    channel_config_set_read_increment(&c, false);
    channel_config_set_write_increment(&c, true);
    dma_channel_configure(rxChan, &c, nullptr, rxfifo, 0, false);
}
...
#if RP2350
void __time_critical_func(PioSPI::DmaCompleteCallback)(CallbackParameter cp, DmaCallbackReason reason) noexcept
#else
void PioSPI::DmaCompleteCallback(CallbackParameter cp, DmaCallbackReason reason) noexcept
#endif
{
	TaskBase::GiveFromISR(static_cast<PioSPI *>(cp.vp)->taskToWake, NotifyIndices::Spi);
    static_cast<PioSPI *>(cp.vp)->taskToWake = nullptr;
}
